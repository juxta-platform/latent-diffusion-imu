...
Shared code lives in the `ldm` package, so scripts run from any working
directory once installed and no longer need `sys.path` manipulation. The
upstream image models need extras that the IMU pipeline does not:
`pip install -e ".[image]"`.

If `pip install -e .` fails inside an older environment with
`canonicalize_version() got an unexpected keyword argument
'strip_trailing_zero'`, the installed `packaging` is shadowing the copy
vendored by a newer `setuptools`. Pin the build tool rather than upgrading
`packaging`, which older `transformers` cannot parse: `pip install
"setuptools<69"`.

### RoNIN dependency

Trajectory metrics (`--ronin_ckpt`) need the [juxta-ronin](../juxta-ronin)
checkout for its network definitions; the sequence loaders in this repo are
self-contained. **The default path assumes `juxta-ronin` is a sibling of this
repository.** Override with `--ronin_root /path/to/juxta-ronin` or by setting
`$JUXTA_RONIN_ROOT`.

### Evaluation frames and sampling

VAE and classifier evaluation resolve the model frame from `--imu_frame`, then
checkpoint metadata, then **world** for legacy checkpoints. VAE training saves
the frame in new checkpoints; use `train_vae_1d.py --imu_frame local` for legacy
device-frame training. The classifier's VAE must match its input frame.

| Model frame | Real HDF5 input | Simulator parquet input | RoNIN input |
| --- | --- | --- | --- |
| Local VAE/classifier | Read device IMU directly | Read `*_local_*`, or undo world export and `phone_rot` when local columns are absent | Rotate raw IMU and VAE reconstruction to world first |
| World VAE/classifier | Rotate with `game_rv` | Read `*_world_*` | Already world |
| LDM, either mode | World frame | World frame | Generated IMU is already world |

`--parquet_from_world` forces the world-to-local conversion for a local model;
`--world_heading` must match the simulator's export heading. For generated
HDF5s, `imu_frame="world"` is detected automatically; `--already_world` handles
older exports without that attribute. A local classifier rejects generated
world IMU. VAE evaluation is intended for real/simulator recordings, not for
evaluating IMU already generated through that VAE by an LDM.

`eval_ldm.py --mode traj` defaults to pure-noise generation. With
`--strength < 1`, the initializer must be synthetic world IMU supplied through
`--sim_input`, `--imu_input`, a pair directory, or a parquet `--input`.
For example, `--input real.hdf5 --imu_input synthetic.parquet --strength 0.5`
uses real ground truth/baseline IMU and synthetic img2img initialization.
For a preprocessed split, img2img requires paired windows with `sim_imu`.
`--mode sim_cond` always starts from noise and **rejects `--strength`**; its
synthetic world IMU is used only as conditioning. LDM loading rejects a VAE
checkpoint explicitly marked local. `compare_ldm_modes.py` retains a default
strength of `0.5` for its trajectory img2img comparison.

`--dry_run` and `--plots_only` do not load checkpoints. New evaluations cache
plot inputs and RoNIN trajectories for replotting. Signal caches retain at least
eight windows (or the requested `--n_plot` count); evaluate with `--n_plot -1`
to retain all windows. Older results without plot caches need one evaluation
with `--force` before they can be replotted. Per-recording classifier windows
are non-overlapping; custom overlapping strides apply only to labeled splits.

### Carrying placements vs classifier classes

Dataset placement folders and classifier output classes are deliberately
decoupled:

* `ldm/evaluation/constants.py` → `CARRYING_PLACEMENTS` / `CARRYING_ALIASES`:
  how a recording was carried, as spelled in dataset paths. Add a placement
  here to give it a canonical name; a folder sitting next to a known placement
  is picked up even without registering it.
* `ldm/data/carrying_labels.py` → `CLASS_NAMES`: what a trained classifier
  predicts. Changing this requires retraining.

So a `head/` folder is preprocessed, evaluated and generated by the VAE and LDM
straight away. A classifier that was never trained on head still classifies
those recordings; they are just excluded from accuracy and confusion matrices.
Use `eval_classifier.py --per_file` to classify every recording in a labeled
directory rather than scoring a train/val split of the known classes.
