"""Evaluate a trained sim-conditioned 1D IMU LDM.

Same features as eval_ldm_1d.py, but conditioning includes the synthetic IMU
latent (VAE-encoded) alongside trajectory. Requires paired real/sim data.

Modes:
  1) Preprocessed paired split:
       --data_dir data/real_sim_pairs_processed --split val
  2) Pair folder (real.hdf5 + synthetic.parquet):
       --pair_dir data/real_sim_imu_pairs/chest/john_chest_ios_corrected \\
       --stats data/real_sim_pairs_processed/stats.pt
  3) Explicit real + sim paths:
       --input .../real.hdf5 --sim_input .../synthetic.parquet \\
       --stats data/real_sim_pairs_processed/stats.pt

Example usage:
  python scripts/eval_ldm_1d_sim_cond.py --config configs/imu/ldm_1d_sim_cond.yaml \\
      --ckpt logs/ldm_1d_sim_cond/.../best-000.ckpt \\
      --first_stage_ckpt logs/vae_1d/.../best-000.ckpt \\
      --data_dir data/real_sim_pairs_processed --outdir outputs/ldm_sim_cond_eval --n_plot 8
"""
...
def _load_process_pair():
    import importlib.util

    path = os.path.join(os.path.dirname(__file__), "preprocess_real_sim_imu_pairs.py")
    spec = importlib.util.spec_from_file_location("preprocess_real_sim_imu_pairs", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.process_pair
...
class PairedWindowDataset(Dataset):
    """Window an aligned real/sim pair and standardize with training stats."""

    def __init__(self, real_path, sim_path, stats_path, window_sec=10, sample_rate=200,
                 latent_length=100, stride_sec=None):
        process_pair = _load_process_pair()
        ...
        # Non-overlapping by default for eval (matches original HDF5WindowDataset)
...
def resolve_pair_paths(pair_dir=None, input_path=None, sim_input=None):
    """Resolve real HDF5 + sim parquet paths from pair_dir or explicit args."""
    if pair_dir is not None:
        real_path = os.path.join(pair_dir, "real.hdf5")
        sim_path = os.path.join(pair_dir, "synthetic.parquet")
    ...
    if input_path is None or sim_input is None:
        raise ValueError(
            "Provide --pair_dir, or both --input (real hdf5) and --sim_input (sim parquet)"
        )
    return input_path, sim_input


def build_dataset(args):
    """Build paired dataset from preprocessed split or on-the-fly pair windows."""
    if args.pair_dir is not None or args.input is not None:
        stats_path = resolve_stats_path(args.stats, args.data_dir)
        real_path, sim_path = resolve_pair_paths(args.pair_dir, args.input, args.sim_input)
        ...
        source = f"pair:{os.path.basename(os.path.dirname(real_path) if args.pair_dir else real_path)}"
        if args.pair_dir:
            source = f"pair:{os.path.basename(args.pair_dir.rstrip('/'))}"
        return dataset, source

    if args.data_dir is None:
        raise ValueError(
            "Provide --data_dir (preprocessed pairs) or --pair_dir / --input+--sim_input"
        )

    stats_path = resolve_stats_path(args.stats, args.data_dir)
    dataset = PairedIMUDataset(args.data_dir, stats_path, split=args.split)
...
def evaluate(model, sampler, loader, device, ddim_steps, ddim_eta, max_batches=None, use_ema=True):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        sim_imu = batch["sim_imu"].float().to(device)
        velocity = batch["velocity"].float().to(device)
        physical_time = batch["physical_time"].float().to(device)
        bs = x.shape[0]

        # Encode sim IMU for conditioning
        sim_posterior = model.encode_first_stage(sim_imu)
        sim_latent = model.scale_factor * sim_posterior.mode()
        cond = {
        ...
        # Trajectory + sim-latent conditioned DDIM generation
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/imu/ldm_1d_sim_cond.yaml")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Pair folder containing real.hdf5 + synthetic.parquet",
    )
    parser.add_argument(
        "--input",
        type=str,
        default=None,
        help="Real .hdf5/.h5 path (requires --sim_input)",
    )
    parser.add_argument(
        "--sim_input",
        type=str,
        default=None,
        help="Synthetic .parquet path (requires --input)",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default=None,
        help="Preprocessed paired dataset dir (train/val + stats.pt)",
    )
    parser.add_argument(
        "--stats",
        type=str,
    ...
    parser.add_argument("--outdir", type=str, default="outputs/ldm_sim_cond_eval")
    ...
    if args.pair_dir is not None:
        metrics["pair_dir"] = os.path.abspath(args.pair_dir)
    elif args.input is not None:
        metrics["input"] = os.path.abspath(args.input)
        metrics["sim_input"] = os.path.abspath(args.sim_input)
    else:
    ...
    title = f"LDM sim-cond gen ({source})"
    plot_overlays(
        inputs.numpy(),
        gens.numpy(),
        args.outdir,
